...
router.get('/', (req, res) => {
  const mediuns = db.getAll('mediuns').sort((a, b) => a.nome.localeCompare(b.nome));
  res.json(mediuns);
});
...
router.post('/', (req, res) => {
  ...
  const medium = db.insert('mediuns', { nome, tipo: tipo || 'Médium', tel: tel || '', endereco: endereco || '', email: email || '', nasc: nasc || '', obs: obs || '', iniciais, data_cadastro: dataCadastro, status: 'Ativo' });
  res.status(201).json(medium);
});

router.put('/:id', (req, res) => {
  const existing = db.getById('mediuns', req.params.id);
  ...
  const medium = db.update('mediuns', req.params.id, {
...
router.delete('/:id', (req, res) => {
  const deleted = db.delete('mediuns', req.params.id);
