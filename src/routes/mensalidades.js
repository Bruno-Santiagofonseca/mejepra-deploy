...
router.post('/', async (req, res) => {
  ...
  if (medium_id) {
    const existing = await db.query('mensalidades', m => 
      m.medium_id === parseInt(medium_id, 10) && m.mes === mes && m.ano === ano
    );
  ...
  const mens = await db.insert('mensalidades', { medium_id: medium_id ? parseInt(medium_id, 10) : null, nome, valor: valor || 0, pago: pago || 0, status: status || 'pendente', mes, ano });
